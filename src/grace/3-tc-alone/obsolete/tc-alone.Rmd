...
We begin by loading the metadata associated with the optimal clasifier as determined in experiment 1.
...
te$type<-ifelse(tmp_[,1]=="both", "tc", tmp_[,1])
...
print(te)
write_csv(te, file.path(tc['SAVE_RESULTS'],  
...
```{r}
ggplot(te %>% filter(dt_class %in% tc$important), aes(x=area, color=dt_class, fill=dt_class)) +
...
```{r}
te <- te %>% ungroup
img_summary <- te %>% filter(dt_class %in% tc$important) %>%
...
freqs <- freqs %>% mutate( tot = `0`+`1`+`2`+`3`+tc) %>%
    mutate(f0=`0`/tot, f1=`1`/tot, f2=`2`/tot, f3=`3`/tot, ftc=tc/tot) %>%
    select(filename, f0:ftc)
M <- as.matrix(freqs[, c('f0', 'f1', 'f2', 'f3', 'ftc')])
...
top_freqs <- freqs %>% 
   ...
   relocate(plate, row, column, replicate, f0, f1, f2, f3, ftc)

top_freqs <- top_freqs %>% left_join(grace, 
                  by=c("plate"="plate", "row"="row", "column"="column"))

klasses <- c('f0', 'f1', 'f2', 'f3', 'ftc')
