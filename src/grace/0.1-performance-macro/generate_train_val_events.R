...
target = "val"   # can be {val | train}
 thresholds = c(0.25, 0.3, 0.35)   #  , 0.3, 0.33, 0.35, 0.4, 0.5, 0.6]
...
PARENT_RAW_OUTPUT=  file.path(CANDESCENCE, "output/grace_macro")
SAVE_RESULTS=       file.path(CANDESCENCE, "performance/grace_macro")
IMAGE_PATH=         file.path(CANDESCENCE, "train-data/grace_macro", target)
DATASET=            file.path(CANDESCENCE, "train-data/grace_macro", target)
...
events_ <- events_ %>% filter(bbox_1 >= 0, bbox_2 >= 0, bbox_3 >= 0, bbox_4 >= 0)
...
c_o <- c("macrophage", "0", "1", "artifact",  "time",  "2",  "unknown", "3", "UFO"  ) 
...
numba = "results_chosenone"   # this was our best one
exp = "chosenone" 
target = "val"   # can be {val | train}
thresholds = c(0.1, 0.2, 0.25, 0.3, 0.35, 0.4, 0.5, 0.65)   #  , 0.3, 0.33, 0.35, 0.4, 0.5, 0.6]
...
SAVE_RESULTS=       file.path(CANDESCENCE, "performance/grace_tc")
IMAGE_PATH=         file.path(CANDESCENCE, "train-data/grace_tc", target)
DATASET=            file.path(CANDESCENCE, "train-data/grace_tc", target)
...
# <-------- isolate all hallucinations.
  ...
  v_blank <- grayscale(readRDS(file.path(root, "src", "grace", "0.1-performance-macro",  "data/v_blank.png")))
...
#  v_blank <- grayscale(readRDS(file.path(root, "src", "grace", "performance", "data/v_blank.png")))
  v_blank <- grayscale(readRDS(file.path(root, "src",  "performance", "0.1-performance-macro", "data/v_blank.png")))
 ...
 # }
  ...
  v_blank <- grayscale(readRDS(file.path(root, "src",  "performance", "data/v_blank.png")))
